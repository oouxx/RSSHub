...
module.exports = async (ctx) => {
    ...
    for(let i = 1; i < data.length; i++){
        ...
        for(let j = 0; j < data[i].list.length; j++){
            if (j === 0) {
                // console.log(data[i].list[j].reason ? data[i].list[j].reason: '')
                // console.log(stockList[i-1][j])
                stockList[i-1][j] = {"reason": data[i].reason ? data[i].reason: ''}
                continue
            }
            // console.log(data[i].list[j].article)
            const name = data[i].list[j].name
            const code = data[i].list[j].code
            const action_info = data[i].list[j].article.action_info
            stockList[i-1][j] = {
                "name": name + ' ' + code,
